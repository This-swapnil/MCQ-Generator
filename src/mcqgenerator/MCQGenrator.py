...
quiz_generation_promt = PromptTemplate(
    input_variables=["text", "number", "grade", "response_json"], template=template
)

quiz_chain = LLMChain(
    llm=llm, prompt=quiz_generation_promt, output_key="quiz", verbose=True
)
...
review_chain = LLMChain(
    llm=llm, prompt=quiz_evaluation_promt, output_key="review", verbose=True
)
...
# Output the results
print("Generated Quiz:", result[0])
print("Quiz Evaluation:", result[1])
